...
func TestRun(t *testing.T) {
	...
	assert.Contains(t, string(b), `return new(A)`)
	assert.Contains(t, string(b), `return new(ARule)`)
	assert.Contains(t, string(b), `return reflect.TypeOf((*AInterface)(nil)).Elem()`)

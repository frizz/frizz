...
func (f *fileDef) repacker(spec ast.Expr, name string, method bool, custom bool) *Statement {
	/*
		func <if method>(p packer) Repack<name></if>(root *frizz.Root, stack frizz.Stack, in <name or spec>) (value interface{}, dict bool, null bool, err error) {
			<...>
		}
	*/
...
func (f *fileDef) customRepacker(g *Group, spec ast.Expr, name string) {
	...
	*/
	...
	g.List(Id("out"), Id("dict"), Id("null"), Err()).Op(":=").Id("in").Dot("Repack").Call(Id("root"), Id("stack"))
...
func (f *fileDef) interfaceRepacker(g *Group, spec *ast.InterfaceType) {
	...
	*/
	...
	g.Return(
		Id("root").Dot("RepackInterface").Call(
			Id("stack"),
			False(),
			Id("in"),
		),
...
func (f *fileDef) mapRepacker(g *Group, spec *ast.MapType) {
	/*
		...
		for k, item := range in {
			v, _, _, err := <repacker>(root, stack, item)
			if err != nil {
				return nil, false, false, err
			}
			out[k] = v
...
func (f *fileDef) sliceRepacker(g *Group, spec *ast.ArrayType) {
	/*
		...
		empty := true
		for i, item := range in {
			v, _, null, err := <repacker>(root, stack, item)
			if err != nil {
				return nil, false, false, err
			}
			if !null {
...
func (f *fileDef) localRepacker(g *Group, spec *ast.Ident) {
	/*
		out, dict, null, err := p.Repack<spec name>(root, stack, in)
		if err != nil {
			return nil, false, false, err
		}
		return out, dict, null, nil
...
func (f *fileDef) nativeRepacker(g *Group, spec *ast.Ident) {
	...
	*/
	...
	g.Return(Qual("frizz.io/frizz", repackFunction).Call(Id("in")))
}
...
func (f *fileDef) structRepacker(g *Group, spec *ast.StructType) {
	/*
		...
		empty := true
		<fields...>
		if v, _, null, err := <repacker>(root, stack, in.<field name>); err != nil {
			return nil, false, false, err
		} else {
			if !null {
				empty = false
	...
	*/
	...
	for _, field := range spec.Fields.List {
		g.If(
			List(Id("v"), Id("_"), Id("null"), Err()).Op(":=").Add(f.repacker(field.Type, "", false, false)).Call(Id("root"), Id("stack"), Id("in").Dot(fieldName(field))),
			Err().Op("!=").Nil(),
		).Block(
			Return(Nil(), False(), False(), Err()),
		).Else().Block(
